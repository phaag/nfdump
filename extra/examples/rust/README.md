...
```
cargo build
DYLD_LIBRARY_PATH=../../../src/libnffile/.libs ./target/debug/read_flows <nfcapd-file> [max]   # macOS
LD_LIBRARY_PATH=../../../src/libnffile/.libs  ./target/debug/read_flows <nfcapd-file> [max]    # Linux
```

`build.rs` points the linker at the in-tree `libnffile` build three
...
directory at run time too, hence the env var above — `build.rs` only
handles link time.
