...
DYLD_LIBRARY_PATH=../../../src/libnfdump/.libs:../../../src/libnffile/.libs ./read_flows <nfcapd-file> [max]   # macOS
LD_LIBRARY_PATH=../../../src/libnfdump/.libs:../../../src/libnffile/.libs  ./read_flows <nfcapd-file> [max]    # Linux
```

The `#cgo CFLAGS`/`#cgo LDFLAGS` lines in `main.go` use cgo's `${SRCDIR}`
substitution to find the in-tree `libnfdump` build three directories up
...
needs both the ABI and its `libnffile` dependency directories at run time,
hence the env var above — the
