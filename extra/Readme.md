...
- `PortTracker.pm` - NfSen plugin demonstrating `nftrack` port statistics.
  Its `nftrack` CLI usage is still valid in 1.8.x.
...
The NfSen-to-InfluxDB integration (`extra/nfsen/*.patch` and the Grafana
dashboard that queried the InfluxDB measurement those patches wrote) has
been removed: it required patching NfSen itself, which is no longer part
of the supported ecosystem here.
